...
\section{Use of Node.js APIs and IPC Mechanisms}
...
\subsection{Node.js API Usage in the Main Process}

The main process, implemented in \texttt{src/electron-main.ts}, executes in a full Node.js environment and is responsible for system-level interactions. Several core Node.js modules and globals are used in this context.
\\\\
File system operations are performed via the \texttt{fs} module. Specifically, \texttt{fs.readFileSync} is used to synchronously load application settings from persistent storage, while \texttt{fs.writeFileSync} serializes and writes updated settings back to disk. These operations are typical for configuration management in desktop applications.
\\\\
Path manipulation is handled using the \texttt{path} module. Calls to \texttt{path.join} are used to construct platform-independent file system paths, ensuring compatibility across operating systems such as Windows, macOS, and Linux.
\\\\
The application also relies on Node.js global objects for environment awareness. The \texttt{process.platform} property is queried to implement platform-specific behavior, for example when handling application lifecycle events. Additionally, the \texttt{\_\_dirname} global is used in conjunction with \texttt{path.join} to resolve absolute paths relative to the location of the executing file.

\subsection{Node.js API Usage in the Preload Script}

The preload script, located at \texttt{src/preload.js}, runs in a privileged context that bridges the gap between the renderer and the main process. While it has access to browser APIs, it also retains controlled access to Node.js functionality.
\\\\
In this file, the Node.js CommonJS module system is used via \texttt{require('electron')}. Although the imported module is provided by Electron, the \texttt{require} function itself is a fundamental Node.js API and illustrates that the preload script operates within a Node.js-enabled environment.

\subsection{Renderer Process Isolation and IPC}

In contrast, the renderer process (including \texttt{main.ts} and Angular components) does not have direct access to Node.js APIs. This restriction is enforced by disabling Node integration (\texttt{nodeIntegration: false}) and is a critical security measure.
\\\\
...
\begin{itemize}
  ...
  \item \textbf{OS-agnostic color picker:} Leveraged Angular's \texttt{<input type="color">} to handle color selection, allowing Electron to manage platform-specific details seamlessly.
...
For Windows, the target installer format is \texttt{Squirrel}, which allows automatic updates and easy installation. For macOS however, \texttt{Squirrel} is not used since this does not properly integrate with modern Apple-enforced macOS code signing, notarization and M-type silicon. The standard electron builder was used instead.
\\\\
